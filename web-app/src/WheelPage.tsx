...
const user = {id: 1, first_name: "John", last_name: "Doe", username: "johndoe", language_code: "en"};

  useEffect(() => {
    if (user?.id) {
      const fetchPrize = async () => {
        try {
          const res = await fetch(`http://localhost:8000/app/prizes/?chat_id=${user.id}`);
  ...
  const handleSpinComplete = async () => {
    try {
      await fetch(`http://localhost:8000/app/mark-spun?chat_id=${user?.id}`, { method: "POST" });
      setHasSpun(true);
    } catch (err) {
  ...
  if (loading)
    return (
      <div style={{ textAlign: "center", padding: "20px" }}>
        ...
        <p style={{ marginTop: "10px", fontSize: "16px",color: "#6c757d" }}>در حال بارگذاری...</p>
      </div>
    );

  return (
    <div className="wheel-container" dir="rtl">
      <h2 style={{ textAlign: "center", fontSize: "24px", color: "#6c757d" }}>{user?.first_name} {user?.last_name} خوش آمدید</h2>
      ...
      {hasSpun ? (
        <div className="prize-card">
          ...
          <div className="prize-image-container">
            <img src={'/images/prize_2.jpg'} alt="جایزه شما" className="prize-image" />
          </div>
